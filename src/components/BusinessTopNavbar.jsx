...
const BusinessTopNavbar = () => {
  ...
  const servicesDropdownItems = [
    { label: 'Software Development & IT Consultancy', action: handleHomeNavigation },
    { label: 'Audit & Business Advisory', action: () => navigate('/business/audit-business-advisory') },
  ];
  ...
  return (
    <>
      ...
      {/* Main Header */}
      <nav 
        className={`fixed top-0 left-0 w-full z-30 bg-white/98 backdrop-blur-md shadow-lg transition-all duration-300 ${
          scrollY > 100 ? 'h-16' : 'h-20'
        }`}
      >
          ...
          <div className="hidden lg:flex items-center gap-8">
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 text-slate-800 font-semibold hover:text-amber-500 transition-all duration-300 whitespace-nowrap group relative"
                >
            ...
            <div className="relative">
              ...
              {activeDropdown === 'services' && (
                <div className="absolute top-full left-0 mt-2 w-72 bg-white/98 backdrop-blur-md rounded-lg border border-amber-500/10 shadow-xl p-2 z-50">
                  {servicesDropdownItems.map((item, index) => (
                    <button
                      key={index}
                      onClick={item.action}
